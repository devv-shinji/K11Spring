...
public class DIController {
	...
	public String myCal(Model model) {
		...
		※ webapp 하위의 resources 디렉토리는 이미지나 js파일 등 리소스를 저장하는 곳이므로 혼동하지 않도록 주의.
		 */
		...
		new 연산자를 통해 생성한 것과 동일하지만 외부 설정파일에서 미리 생성한 것을 주입(Injection)받은것임에 유의할 것.
		 */
	...
	public String bmiCal(Model model) { 
		...
		//인자1: 주입받을 bean의 이름, 인자2: bean의 타입
		/*
			사용법: 
		...
		//주입받은 빈을 통해 멤버메소드 호출 및  model 객체에 정보저장
	...
	public ModelAndView myAvengers() {
		...
		대응하는 것이 ApplicationContext이다. 
		다른 말로는 IoC컨테이너, 스프링컨테이너 혹은 BeanFactory로 부르기도 한다.
		...
		//아이언맨 빈을 주입받은 후 정보출력을 위해 setter를 호출함. (QQQ 설명이 좀 이상한뎅?)
