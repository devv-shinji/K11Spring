...
기본 패키지로 지정된 com.kosmo.k11spring을 스캔하여 컨트롤러 클래스를 찾는다.
...
@RequestMapping	
	요청명을 매핑한다. 요청명은 서블릿과 마찬가지로 컨택스트 루트를 제외한 나머지경로명으로 
	제외한 나머지경로명으로 이루어진다. 요청명의 매핑정보로 메소드를 호출하게 되므로 메소드명은 큰 의미가 없다.
...
public class FormController {
	...
	/*
	폼값받기1] 파라미터로 전달된 값을 HttpServletRequest객체를 통해서 받는다.
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		ViewResolver가 경로를 조립하여 해당 View를 웹브라우저에 출력하게 된다.
	...
	폼값받기2] @RequestParam 어노테이션으로 폼값받기
		...
		이와 같이 하면 해당 메소드내에서 변수명을 그대로 사용할 수 있다.
	 */
	...
	폼값받기3] 커맨드객체를 이용해서 폼값 한번에 받기
		조건1 : 쿼리스트링으로 전달되는 파라미터의 개수와 폼값을 저장 할 객체(DTO 혹은 VO)의 
			멤버변수의 개수가 동일할 때 사용가능함.
		조건2 : 커맨드객체를 사용할 때는 클래스명 앞글자를 소문자로 바꾼 형태의 매개변수를 사용해야 한다.
			예) MemberDTO -> memberDTO
		조건3 : 커맨드객체를 만들 때 파라미터의 개수는 달라도 상관없으나 
			파라미터의 이름과 멤버변수의 이름은 반드시 동일해야 하고 이에 해당하는 getter/setter가 생성되어야 한다.
	...
	폼값받기4] @PathVariable어노테이션으로 폼값받기
		...
		단 웹브라우저는 요청명을 경로로 인식하므로 리소스(이미지 등)를 사용할 때는 경로에 주의해야 한다. 
		파라미터의 개수가 틀릴 경우 404에러가 발생한다.
	 */
