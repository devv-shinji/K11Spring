...
public class MemberValidator implements Validator{
	...
	/*
	supports() 메소드
		: 매개변수로 전달되는 객체가 유효성검증을 지원할 수 있는 커맨드객체인지 여부를 판단한다. (자동호출됨)
		만약 해당 메소드를 통과하지 못하면 유효성검증을 진행하는 validate()메소드는 호출되지 않는다.
		해당 메소드는 자동으로 호출된다.
	 */
	@Override
	...
	public void validate(Object obj, Errors errors) {
		...
		if(member_id==null || member_id.trim().isEmpty()) {
			...
			//검증에 실패한 경우 해당메소드를 통해 결과반환
			errors.rejectValue("id", "idError");
		}
		
		//내장메소드를 통한 검증: 패스워드(pw). 검증 실패 시 자동으로 reject되므로 rejectValue메소드가 필요없다.
