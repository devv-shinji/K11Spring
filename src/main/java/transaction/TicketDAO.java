...
public class TicketDAO {
	...
	//티켓구매와 결제를 위한 메소드
	public void buyTicket(final TicketDTO dto) { //익명클래스에서 매개변수를 사용하기 위해 final로 선언함
		...
		/*
		※ 테스트시에는 아래 두개의 업무처리 로직을 try문 밖으로 이동한 후  구매를 해본다.
		6장 이상인 경우 pay테이블에는 insert되지만, ticket테이블에는 제약조건 위배로 insert되지 않는다.
		 */
		...
		try {
			...
			//티켓 구매 처리
			/*
			check제약조건에 의해 countNum이 5를 초과하는 경우에는 제약조건위배로 에러(예외)가 발생된다.
			 */
			template.update(new PreparedStatementCreator() {
				@Override
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
			...
			/*
			티켓 구매개수가 5이하인 경우 정상처리되어 모든 작업이 commit된다.
			 */
			...
			/*
			티켓 구매개수가 6이상인 경우 예외가 발생되어 모든 작업은 취소(rollback)처리된다.
			 */
